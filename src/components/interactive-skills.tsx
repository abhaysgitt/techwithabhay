...
const SkillCategory: React.FC<SkillCategoryProps> = ({ name, skills, icon, isActive, onClick, count }) => {
  return (
    <button 
      ...
      className={cn(
        "text-xs px-3 py-1.5 rounded-xl transition-all duration-300 font-medium flex items-center gap-1.5",
        isActive
          ? "bg-[#626364]  text-white shadow-md"   // dark blue
    ...
    >
      ...
      <span className={cn(
        "inline-flex items-center justify-center rounded-xl text-xs w-5 h-4", // Smaller counter circle
        isActive 
